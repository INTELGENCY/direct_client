...
function Sectiontwo() {
  ...
  const route = isAcademia ? "/academia/signup" : "/industry/signup";
  ...
  const handleLogin = async (values) => {
    try {
      ...
      const res = await axios.post(keys.api + "industry/signin", valuess);
      ...
      if (res.status === 200) {
        ...
        dispatch(
          setAlert({ status: "error", text: data?.result || "Couldn't Login!" })
        );
      }
      setIsLoadng(false);
    } catch (e) {
                  ...
                  >
                    <Grid item xs={6}>
                      <FlexBox>
                        {/* <LoginText> */}
                        <LoginLink
                          onClick={() => navigate("/industry/resetpassword")}
                        >
                          Forgot Password?
                        </LoginLink>
                        {/* </LoginText> */}
